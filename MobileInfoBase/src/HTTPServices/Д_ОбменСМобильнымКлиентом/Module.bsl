...
//Итоговая работа по модулю Мобильная разработка в 1С
	...
	КонецЕсли;
		...
		//ЧтениеJSON.УстановитьСтроку(ОтветСервераСтрока); //Сюда нужно добавить тело запроса
		...
		ОбъектДоставка = СсылкаДоставка.ПолучитьОбъект();
			...
			Если ОбъектДоставка = Неопределено Тогда
				...
				Ответ.УстановитьТелоИзСтроки(СтрШаблон("Переданный документ отсутствует в базе: %1", Доставка.Ссылка));
			...
			Иначе
				...
				//Нужно создать обработку заполнения документа и регистра
